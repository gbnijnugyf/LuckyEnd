...
func UserAddDesire(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(desireFormView); err != nil {
		...
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "绑定数据模型失败", err))
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "添加愿望失败", err))
		return
	}
...
func UserLightDesire(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(lightFromView); err != nil {
		log.Errorf("request param error %+v", errors.WithStack(err))
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "数据模型绑定错误", err))
		return
	}
	UserID := c.MustGet("user_id").(int)
	...
	// 	c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "bad params", err))
	// 	return
	// }
	checkID, err := model.GetUserID(&DesireID)
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "查询错误", err))
		return
	}
	if checkID == UserID {
	...
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "发送邮件失败", err))
		return
	}
	go func() {
...
func GetUserCreateDesires(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "查询失败", err))
		return
	}
...
func GetUserLightDesires(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "查询失败", nil))
		return
	}
...
func GetUserDesireByType(c *gin.Context) {
	...
	if !ok {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "type is empty", nil))
		return
	}
	desireType, err := strconv.Atoi(Type)
	if err != nil {
		log.Errorf("cannot convert type into int err :%+v", err)
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "type is not right", err))
		return
	}
...
func DeleteUserDesire(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "desire_id is not right", err))
		return
	}
	checkID, err := model.GetUserID(&desireID)
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "删除失败", err))
		return
	}
	if UserID != checkID {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "不能删除不是你的愿望哦", nil))
		return
	}
	...
	if res != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeSuccess, "删除失败", res))
		return
	}
	go func() {
...
func CancelUserLight(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&json); err != nil {
		log.Errorf("request param error %+v", errors.WithStack(err))
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "绑定数据失败", err))
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "取消点亮失败,无法验证身份", nil))
		return
	}
	if UserID != checkID {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "无法取消不是你点亮的愿望哦", nil))
		return
	}
	desireContent, err := model.DesireContentByID(&desireID)
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "取消点亮失败", err))
		return
	}
	email, err := model.GetEmail(&desireID)
	if err != nil {
	...
	if res != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "取消点亮失败", res))
		return
	}
...
func AchieveUserDesire(c *gin.Context) {
	...
	checkID, err := model.GetUserID(&desireID)
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "实现愿望失败", err))
		return
	}
	if UserID != checkID {
		email, err := model.GetEmail(&desireID)
	...
	if res != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "实现失败", res))
		return
	}
...
func DesireDetail(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, helper.ApiReturn(common.CodeError, "获取信息失败", err))
		return
	}
